...
def add_obj(objlIN, label, coord, obj_id=None, tomo_idx=None, orient=(None,None,None), cluster_size=None, cluster_conf=None):   #   REPIC_PATCH
    obj = {
        'tomo_idx': tomo_idx,
        'obj_id'  : obj_id  ,
        'label'   : label   ,
        ...
        'cluster_conf':cluster_conf,    #   REPIC_PATCH
    }
...
def write_xml(objlIN, filename):
    ...
    for idx in range(len(objlIN)):
        ...
        cconf = objlIN[idx]['cluster_conf']             #   REPIC_PATCH
        ...
        if not cconf is None:                           #   REPIC_PATCH
            obj.set('cluster_conf', str(cconf))         #   REPIC_PATCH
