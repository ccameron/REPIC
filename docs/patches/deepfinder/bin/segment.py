...
def main():
    ...
    parser.add_argument('-t', action='store',
                        dest='path_tomo', help='path to tomogram')
    parser.add_argument('-w', action='store',
                        dest='path_weights', help='path to network weights')
    parser.add_argument('-c', action='store', dest='nclass',
                        type=int, help='number of classes')
    parser.add_argument('-p', action='store', dest='psize',
                        type=int, help='patch size, must be multiple of 4')
    parser.add_argument('-o', action='store',
                        dest='path_output', help='output path')
    parser.add_argument('-bin', action='store_true',
                        help='bin resulting segmentation')
    ...
    if no_args:  # if no args are passed, then open GUI
        ...
        seg = Segment(Ncl=args.nclass,
                      path_weights=args.path_weights, patch_size=args.psize)
        ...
        # Save labelmap:
        s = os.path.splitext(args.path_output)                                  #   REPIC_PATCH
        cm.write_array(scoremaps, ''.join([s[0], "_scoremaps.h5"]))             #   REPIC_PATCH
        ...
        if args.bin:
            ...
            cm.write_array(scoremapsB, ''.join([s[0], "_scoremaps_binned.h5"])) #   REPIC_PATCH
