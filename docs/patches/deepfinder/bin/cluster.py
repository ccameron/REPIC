...
def main():
    ...
    if no_args:  # if no args are passed, then open GUI
        ...
        s = os.path.splitext(args.path_lmap)                        #   REPIC_PATCH
        try:                                                        #   REPIC_PATCH
            smap = cm.read_array(''.join([s[0], "_scoremaps.h5"]))  #   REPIC_PATCH
        except OSError:                                             #   REPIC_PATCH
            smap = None                                             #   REPIC_PATCH

        # Initialize clustering task:
        clust = Cluster(clustRadius=args.cradius)

        # Launch clustering (result stored in objlist)
        objlist = clust.launch(lmap, smap)                          #   REPIC_PATCH
